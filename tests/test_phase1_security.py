...
# ---------------------------------------------------------------------------
# Bootstrap: add backend/app to Python path (mirrors pytest.ini settings)
# ---------------------------------------------------------------------------
...
class TestIngestRouteSecurity:
    ...
    def test_ingest_imports_get_current_admin_user(self):
        src_text = (BACKEND / "app" / "api" / "routes" / "ingest.py").read_text()
...
class TestAuthService:
    def _get_source(self):
        return (BACKEND / "app" / "services" / "auth_service.py").read_text()
